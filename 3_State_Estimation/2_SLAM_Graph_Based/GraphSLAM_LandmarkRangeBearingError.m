...
sigma_range = 0.5; % landmark relative ranging %static,if changing then for every pose-landmark estimate based on range
sigma_bearing = deg2rad(10.0); % landmark relative bearing %static,if changing then for every pose-landmark estimate based on range
R = [sigma_range^2,    0;
     0            ,    sigma_bearing^2];
R_inv = inv(R);
...
if num_landmarks >= 1
  ...
  % Damping
  landmark_error_terms_size = 2;
  lambda = 1e-3;
  Error = zeros(3*num_poses+num_poses*landmark_error_terms_size*num_landmarks , 1);  % 3 error terms for pose-pose, 2 error term for pose-landmark (bearing-only model)
    ...
    for t=0:num_poses-1
      ...
      elseif t >= 1
        ...
        e_ij = [e_xy;...
          ...
          % Damping
          Error(X_poses_index + (t)*3 : X_poses_index + (t+1)*3-1) = e_ij; 
      ...
      for n=0:num_landmarks-1
          ...
          z_ij = [ra_t(1); ...
                  ra_t(2)]; % (t)->(n)-th relative range-bearing measurement from sensor model
          dx = -ksi_estim_t(1) + mn_t(1);
          dy = -ksi_estim_t(2) + mn_t(2);
          dr = norm([dx,dy]);
          da = (atan2(dy,dx)-(ksi_estim_t(3)));
          e_ij = [-(z_ij(1))+(dr);...  
                  wrapToPi(-(z_ij(2))+(da))];  % the Euclidian equivalent of SE(3) formulation (Z_ij^-1 * (X_i^-1 * X_j))

            % Damping
            Error(X_landmarks_index + (t*(num_landmarks)*landmark_error_terms_size) + (n*landmark_error_terms_size) : X_landmarks_index + (t*(num_landmarks)*landmark_error_terms_size) + (n+1)*landmark_error_terms_size-1) = e_ij; 

          % jacobian of error:
          J_ij = [ -dx/dr     , -dy/dr     ,  0 ,  dx/dr     ,  dy/dr;...
                   -dy/(dr^2) ,  dx/(dr^2) , -1 ,  dy/(dr^2) , -dx/(dr^2)];
...
%     x_star = - inv(H) * b;
    ...
    for tt=0:num_poses-1 
      ...
      % Damping
      Error_new = zeros(3*num_poses+num_poses*2*num_landmarks , 1);  % 3 error terms for pose-pose, 2 error term for pose-landmark (bearing-only model)
      %Error_new = zeros(3*num_poses+2*num_landmarks , 1);  % 3 error terms for pose-pose, 2 error term for pose-landmark (bearing-only model)
      for t=0:num_poses-1
        if t == 0
          % nothing already zeros in Error_new
        elseif t >= 1
          u_t = u_inputs{1 + t}.u_t; % t-th control input (corresponds to t-1 applied robot control)
          ksi_estim_tm1 = X(X_poses_index + (t-1)*3 : X_poses_index + (t)*3-1);
          ksi_estim_t   = X(X_poses_index + (t)*3 : X_poses_index + (t+1)*3-1);
          z_ij = [u_t(1)*cos(ksi_estim_tm1(3))*Dt; ...
                  u_t(1)*sin(ksi_estim_tm1(3))*Dt; ...
                  u_t(2)*Dt]; % (t-1)->(t)-th relative transformation "virtual measurement" from motion model
          e_ij = -(z_ij) + (-ksi_estim_tm1 + ksi_estim_t);  % the Euclidian equivalent of SE(3) formulation (Z_ij^-1 * (X_i^-1 * X_j))
          Error_new(X_poses_index + (t)*3 : X_poses_index + (t+1)*3-1) = e_ij; 
        end
        % for every landmark of our graph
        z_mi = z_measurements{1 + t}.z_mi; % actual measurements z for t-th robot pose -> landmark_i:1...n
        for n=0:num_landmarks-1
          ...
          e_ij = [-(z_ij(1))+(dr);...  
                  wrapToPi(-(z_ij(2))+(da))];  % the Euclidian equivalent of SE(3) formulation (Z_ij^-1 * (X_i^-1 * X_j))
          Error_new(X_landmarks_index + (t*(num_landmarks)*landmark_error_terms_size) + (n*landmark_error_terms_size) : X_landmarks_index + (t*(num_landmarks)*landmark_error_terms_size) + (n+1)*landmark_error_terms_size-1) = e_ij; 
        end
      end
      if norm(Error) < norm(Error_new)
        X = X_old;
        lambda = lambda * 2
      else
        lambda = lambda / 2
